...
static int http_rest_get_wl5(http_request_t* request) {
	...
	poststr(request,
		...
		"</div>"
		"<h2>Raw PWM</h2>"
		"<div class='row'><span>Taajuus</span><span style='font-size:.78rem;color:#9c9' id='freqHz'>-</span></div>"
		"<div class='row'><input type='number' id='fPer' min='1' max='65535' style='width:76px' placeholder='period'>"
		"<input type='number' id='fPre' min='0' max='65535' style='width:76px' placeholder='presc'>"
		"<button id='fSet' class='sg' style='flex:1'>Aseta FREQ</button></div>"
		"</div>"
	...
	poststr(request,
		...
		".then(function(r){return r.text();}).then(applyStatus).catch(function(){});modeRefresh();freqRefresh();}"
		...
		"function calcHz(per,pre){return Math.round(16000000/((per+1)*(pre+1)));}"
		...
		"document.getElementById('fPer').value=per;document.getElementById('fPre').value=pre;"
		"document.getElementById('freqHz').textContent=calcHz(per,pre)+' Hz  (period '+per+', presc '+pre+')';}).catch(function(){});}"
		"document.getElementById('fSet').addEventListener('click',function(){"
		"var per=document.getElementById('fPer').value,pre=document.getElementById('fPre').value;"
		"send('FREQ:'+per+':'+pre);setTimeout(function(){freqRefresh();modeRefresh();},250);});");
	poststr(request,
